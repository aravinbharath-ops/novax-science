...
verifyForm.addEventListener('submit', async event => {
  ...
  try {
    ...
    if (!response.ok || !data.verified) {
      ...
      verifyResult.innerHTML = `<strong>Certificate not found.</strong><br>${data.message || 'Check the serial number and try again.'}`;
      return;
    }

    window.currentCertificateRecord = data.certificate;
    verifyResult.className = 'verify-result success';
    verifyResult.innerHTML = `<strong>Certificate verified.</strong><br>${data.certificate.product} · ${data.certificate.strength} · ${data.certificate.analysisDate}<br><button type="button" class="inline-certificate-button" data-open-current-certificate>View certificate</button>`;
  } catch (error) {
    verifyResult.className = 'verify-result';
